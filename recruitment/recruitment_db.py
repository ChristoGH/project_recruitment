    ...
    def link_job_company(self, job_id: int, company_id: int) -> None:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO job_companies (job_id, company_id) VALUES (?, ?)",
                    (job_id, company_id)
                )
                conn.commit()
        except sqlite3.Error as e:
            raise DatabaseError(f"Failed to link job and company: {str(e)}")

    def link_job_agency(self, job_id: int, agency_id: int) -> None:
        """Link a job to an agency."""
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO job_agencies (job_id, agency_id) VALUES (?, ?)",
                    (job_id, agency_id)
                )
                conn.commit()
        except sqlite3.Error as e:
