...
from recruitment.models.url_models import URL
    ...
    def process_urls(self, urls: List[Dict[str, str]]) -> None:
        ...
        for url_data in urls:
            try:
                ...
                self.db.update_url_processing_status(
                    url_id=url_id, status="pending", error_message=None
                )
                ...
                logger.error(
                    f"Error processing URL {url_data.get('url', 'unknown')}: {str(e)}"
                )
                continue
