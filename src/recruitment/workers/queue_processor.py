...
from recruitment.models.url_models import URLProcessingResult
from recruitment.utils.web_crawler import crawl_website_sync_v2


async def process_urls_from_queue(
    urls: List[str], db: Optional[RecruitmentDatabase] = None
    ...
    for url in urls:
        try:
            ...
            if result.success:
                ...
                results.append(
                    URLProcessingResult(url=url, success=True, content=result.markdown)
                )
            else:
                results.append(
                    URLProcessingResult(url=url, success=False, error=result.error)
                )
